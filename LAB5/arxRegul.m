function  [Lambda, R, obj, df] = arxRegul(data,Orders,opt)
%ARXREGUL Determine regularization matrix for an ARX model.
%
%  [LAMBDA, R] = ARXREGUL(DATA, ORDERS)
%    Computes the scaling Lambda and regularization matrix R such that the
%    ARX estimate is obtained by solving inv(J'J + LAMBDA*R)*(J'*y) where J
%    is the regressor matrix and y is the measured output. 
%
%    DATA is time- or frequency-domain IDDATA object with nonzero sample
%    time.
%
%    ORDERS is the matrix of ARX model orders [na nb nk]. See "help arx"
%    for more information.
%
%    The returned values can be used to configure the regularization
%    options for ARX model estimation. See "Regularization" option under
%    the option set returned by ARXOPTIONS.
%
%  [LAMBDA, R] = ARXREGUL(DATA, ORDERS, OPTIONS)
%    specifies the choice of regularization kernel I/O offsets and other
%    estimation options. See "help arxRegulOptions" for more information.
%
%  Example: Obtain a regularized estimate of an ARX model of order [10 10 1]
%  using 'TC' as the regularization kernel.
%     load iddata1 z1 % estimation data
%     Orders = [10 10 1];
...
%
%  Author: Mattia Zorzi December 2019
...
%% kernel tipe
if strcmp(string(opt.RegulKernel),'DI')
    ktype='DI';
end
if strcmp(opt.RegulKernel,'TC')
    ktype='TC';
end
if strcmp(opt.RegulKernel,'SS')
    ktype='SS';
end
...
[lambda, obj] = fmincon(@(lambda) loglikb(lambda,Phi,yn,nv,p,lab,ktype),lambda_init,[],[],[],[],lb,[],[],opt);
                                          
%% kernel 
switch lab
    case 'ny'
        B=kernel(lambda(1),lambda(2),p,ktype);
        K= B*B'+10^-10*eye(p);
    case 'yn'
        B=kernel(lambda(1),lambda(2),p,ktype);
        K= B*B'+10^-10*eye(p);
    case 'yy'
        B1 = kernel(lambda(1),lambda(3),p,ktype);
        B2 = kernel(lambda(2),lambda(4),p,ktype);
        K=zeros(2*p,2*p);
        K(1:p,1:p)= B1*B1'+10^-10*eye(p);
        K(p+1:2*p,p+1:2*p)= B2*B2'+10^-10*eye(p);
end
...
switch lab
    case 'ny'
        obj = obj+0.5*(nv^-1*y'*y+(size(y,1)-p)*log(nv));
        df=trace(Phi*(Phi'*Phi+nv*K^-1)^-1*Phi');
    case 'yn'
        obj = obj+0.5*(nv^-1*y'*y+(size(y,1)-p)*log(nv));
        df=trace(Phi*(Phi'*Phi+nv*K^-1)^-1*Phi');
    case 'yy'
        obj = obj+0.5*(nv^-1*y'*y+(size(y,1)-2*p)*log(nv));
...
function obj=loglikb(lambda,Phi,y,nv,p,type,ktype)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function B = kernel(lambda,b,T,ktype)
    K=zeros(T,T);
    switch ktype
        case 'DI'
            for k=1:T
                K(k,k)=lambda*exp(-b*k);
            end
        case 'TC'
            for k=1:T
                for j=1:T
                    K(k,j)=lambda*min(exp(-b*j),exp(-b*k));
                end
            end
        case 'SS'
            for k=1:T
                for j=1:T
                    K(k,j)=lambda*(exp(-b*(k+j))*exp(-b*max(k,j))/2-exp(-3*b*max(k,j))/6);
                end
            end          
    end
    B = chol(K)';
